...
YOLO10Detector::YOLO10Detector(const std::string &modelPath, const std::string &labelsPath, bool useGPU) {
    ...
    // bool trtAvailable = std::find(availableProviders.begin(), availableProviders.end(), "TensorrtExecutionProvider") != availableProviders.end();
    // int deviceId = 0; // Default device ID for CUDA

    // bool trtPluginsInited = initLibNvInferPlugins(nullptr, "");
    // if (!trtPluginsInited) {
    //     std::cerr << "initLibNvInferPlugins has failed!" << std::endl;
    // }
    ...
    // if (useGPU && trtAvailable) {
    //     std::cout << "Inference device: TensorRT (via GPU)" << std::endl;
